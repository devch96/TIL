...
- 애너그램을 이루는 단어들을 정렬하면 모두 같은 값을 갖게 되기 때문
- 정렬한 값을 보관하기 위한 맵을 선언하고, 하나의 키에 애너그램인 여러 개의 값이 추가될 수 있기 때문에 값은 리스트 형태가 됨
...
- 자바 구현에서는 문자열을 char[] 형태의 문자 배열로 변환한 다음 Arrays.sort()로 정렬하고 String으로 변환하는 과정을 거치지만 코틀린에서는
  한 줄로 처리가능함
- 자바에서는 키 존재 유무를 확인해서 키가 없으면 빈 리스트를 삽입하고 키에 해당하는 리스트에 값을 추가하였으나 코틀린에서는 getOrPut() 메서드를 이용해 간결하게
  처리할 수 있음
...
- 팰린드롬 판별만 하면 된다는 점에 착안해서 매칭이 될 때 중앙을 중심으로 점점 확장해 나가면서 가장 긴 팰린드롬을 판별
- 투 포인터가 슬라이딩 윈도우처럼 계속 앞으로 전진해나감
  - 윈도우에 들어온 문자열이 팰린드롬인 경우 그 자리에서 멈추고 투 포인터는 점점 확장해나가는 것
  - 팰린드롬은 홀수, 짝수 둘다 가능(aba, bb)
...
public String longestPalindrome(String s) {
    ...
    for (int i = 0; i < len - 1; i++) {
        extendPalindrome(s, i, i + 1); // 2칸 짜리 투 포인터
        extendPalindrome(s, i, i + 2); // 3칸 짜리 투 포인터
    }
